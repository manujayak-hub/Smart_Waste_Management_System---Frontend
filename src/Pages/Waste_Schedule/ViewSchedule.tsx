...
const ViewSchedule: React.FC = () => {
  ...
  const fetchSchedules = async () => {
    if (uid) {
      try {
        const fetchedSchedules = await ScheduleService.fetchSchedulesByUserId(
          uid
        );
        console.log(uid);
          ...
          {schedules.map((schedule) => (
            ...
            >
              ...
              <p className="text-gray-700 mb-2">
                <strong>Date:</strong> {schedule.cdate}
              </p>
              <p className="text-gray-700 mb-2">
                <strong>Area:</strong> {schedule.area}
              </p>
              <p className="text-gray-700 mb-2">
                <strong>Timeslot:</strong> {schedule.timeslot}
              </p>
              <p className="text-gray-700 mb-2">
                <strong>Status:</strong>{" "}
                {schedule.jobstatus ? "Completed" : "Pending"}
              </p>
